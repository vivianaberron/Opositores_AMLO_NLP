...
def cargar_y_procesar_texto_excel(ruta_archivo, columna_texto):
    ...
    # Combinar todos los textos en la columna especificada en un solo string
    texto = " ".join(df[columna_texto].dropna().astype(str).tolist())
    
    # Eliminar saltos de línea y caracteres extraños
    texto = texto.replace('\n','').replace('\r', ' ').strip()
    
    # Procesar el texto y realizar tokenización/lematización
    doc = nlp(texto)
...
def procesar_texto_y_guardar_tópicos_excel(ruta_archivo, columna_texto, num_topics=15):
    tokens = cargar_y_procesar_texto_excel(ruta_archivo, columna_texto)
...
# Ejemplo de uso
procesar_texto_y_guardar_tópicos_excel("./data/Frecuencias por año/frecuencias_globales_ngramas_2019.xlsx", "N-grama", num_topics=10)
